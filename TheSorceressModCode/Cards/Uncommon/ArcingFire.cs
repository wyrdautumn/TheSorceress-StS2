    ...
    TargetType.AnyEnemy)
{
        ...
        CardPlay play)
    {
        ...
        AttackCommand attackCommand = await DamageCmd.Attack(cardSource.DynamicVars.Damage.BaseValue).FromCard((CardModel) cardSource, play).Targeting(play.Target).WithHitFx(vfx:"vfx/vfx_fire_burst",sfx:"event:/sfx/characters/attack_fire").WithAttackerAnim("Cast",0.2f).Execute(choiceContext);
